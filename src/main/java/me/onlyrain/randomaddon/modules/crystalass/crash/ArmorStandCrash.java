...
public class ArmorStandCrash extends Module {
    ...
    public ArmorStandCrash() {
        super(RandomAddon.CATEGORYC, "Armor Stand Crash", "Attempts to crash the server using armor stands.");
    ...
    public void onActivate() {
        if (!mc.isInSingleplayer()) {
            if (this.mc.player != null && !this.mc.player.getAbilities().creativeMode) {
                this.error("You must be in creative mode to use this.", new Object[0]);
                this.toggle();
            }
        } else {
