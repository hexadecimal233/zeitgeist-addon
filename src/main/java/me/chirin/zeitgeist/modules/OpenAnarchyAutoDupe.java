...
import me.chirin.zeitgeist.utils.ImmediateBlockIterator;
...
public class OpenAnarchyAutoDupe extends Module {
    ...
    public OpenAnarchyAutoDupe() {
        super(Zeitgeist.CATEGORY, "OpenAnarchyAutoDupe", "description");
    }
    ...
    public void onDeactivate() {
        if (redstone != null && mc.world != null && mc.player != null && mc.world.isAir(redstone) && switchToItem(Items.REDSTONE, true, true, null)) {
                    ...
                    if (pistons.stream().anyMatch(s -> entity.getBlockPos().equals(s.offset(mc.world.getBlockState(s).get(Properties.FACING))))) {
                        if (((ItemFrameEntity) entity).getHeldItemStack().isEmpty()) {
                            ...
                            if (duperedstonewhenno.get() && (InvUtils.find(Items.REDSTONE).count() <= redstonedupeamount.get() || (redstonedupe && InvUtils.find(Items.REDSTONE).count() <= 60))) {
                                redstonedupe = true;
                                if (switchToItem(Items.REDSTONE, true, true, this)) {
                                ...
                                redstonedupe = false;
                                if (switchToItem(InvUtils.find(stack -> itemtoplace.get().itemPredicate.test(stack.getItem())), true, true, (timer > shulkerpickup.get() ? this : null), itemtoplace.get().item)) {
                                    mc.interactionManager.interactEntity(mc.player, entity, Hand.MAIN_HAND);
    ...
    public void onRender(Render3DEvent event) {
        ...
        if (currentframe != null && (stage == Stage.Placeshulker || stage == Stage.Waitforshulker) && renderframes.get()) {
            ...
            if (direction != null) {
                renderQuad(currentframe.getBlockPos(), direction.getOpposite(), event, new Color(1f, 0f, 0f, 0.5f));
            }
        }
    ...
    private boolean switchToItem(FindItemResult item, boolean quickmove, boolean force, Module thismodule, Item iteminmessage) {
        ...
        if (item.found()) {
            if (item.isHotbar()) {
                ...
                if (empty.found() && empty.isHotbar()) {
                    if (quickmove && !(mc.currentScreen instanceof GenericContainerScreen))
                        InvUtils.quickMove().from(item.slot()).toHotbar(empty.slot());
    ...
    private boolean switchToItem(Item item, boolean quickmove, boolean force, Module thismodule) {
    ...
    private void renderQuad(BlockPos block, Direction direction, Render3DEvent event, Color color) {
        int x = block.getX();
        int y = block.getY();
        int z = block.getZ();
