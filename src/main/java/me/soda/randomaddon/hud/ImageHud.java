package me.soda.randomaddon.hud;
...
public class ImageHud extends HudElement {
    ...
    public void render(HudRenderer renderer) {
        ...
        if (hFlip.get() && vFlip.get())
            new R2DTexExtra(Renderer2D.TEXTURE).texQuadHVFlip(this.x, this.y, this.getWidth(), this.getHeight(), colorFilter.get());
        else if (hFlip.get())
            new R2DTexExtra(Renderer2D.TEXTURE).texQuadHFlip(this.x, this.y, this.getWidth(), this.getHeight(), colorFilter.get());
        else if (vFlip.get())
            new R2DTexExtra(Renderer2D.TEXTURE).texQuadVFlip(this.x, this.y, this.getWidth(), this.getHeight(), colorFilter.get());
    ...
    private static class R2DTexExtra {

        private final Renderer2D instance;

        public R2DTexExtra(Renderer2D instance) {
            this.instance = instance;
        }

        public void texQuadHFlip(double x, double y, double width, double height, Color color) {
            instance.triangles.quad(
                instance.triangles.vec2(x + width, y).vec2(0, 0).color(color).next(),
                instance.triangles.vec2(x + width, y + height).vec2(0, 1).color(color).next(),
                instance.triangles.vec2(x, y + height).vec2(1, 1).color(color).next(),
                instance.triangles.vec2(x, y).vec2(1, 0).color(color).next()
            );
        }

        public void texQuadVFlip(double x, double y, double width, double height, Color color) {
            instance.triangles.quad(
                instance.triangles.vec2(x, y + height).vec2(0, 0).color(color).next(),
                instance.triangles.vec2(x, y).vec2(0, 1).color(color).next(),
                instance.triangles.vec2(x + width, y).vec2(1, 1).color(color).next(),
                instance.triangles.vec2(x + width, y + height).vec2(1, 0).color(color).next()
            );
        }

        public void texQuadHVFlip(double x, double y, double width, double height, Color color) {
            instance.triangles.quad(
                instance.triangles.vec2(x + width, y + height).vec2(0, 0).color(color).next(),
                instance.triangles.vec2(x + width, y).vec2(0, 1).color(color).next(),
                instance.triangles.vec2(x, y).vec2(1, 1).color(color).next(),
                instance.triangles.vec2(x, y + height).vec2(1, 0).color(color).next()
            );
        }
    }
}
